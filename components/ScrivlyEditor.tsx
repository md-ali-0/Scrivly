...
export const ScrivlyEditor: React.FC<ScrivlyEditorProps> = ({
  ...
  const updateActiveStates = useCallback(() => {
    ...
    // Check lists
    activeStates.bulletList = document.queryCommandState("insertUnorderedList")
    activeStates.numberList = document.queryCommandState("insertOrderedList")
    ...
    while (node && node !== editorRef.current) {
      if (node.nodeType === Node.ELEMENT_NODE) {
        ...
        if (element.tagName === "BLOCKQUOTE") activeStates.blockquote = true
      }
  ...
  const handleInput = useCallback(() => {
    ...
    onStateChange?.({
      content,
      activeFormats: new Set(Object.keys(isActive).filter((key) => isActive[key])),
      currentBlock,
      isDarkMode,
      isFullscreen,
    })
  ...
  const toolbarActions = useMemo(
    () => ({
      ...
      formatBlock: (format: BlockFormat) => execCommand("formatBlock", format),
      ...
      fontSize: (size: FontSize) => execCommand("fontSize", "7") && execCommand("fontName", size),
  ...
  const handleTableOperation = useCallback(
    (operation: string, element?: HTMLElement) => {
        ...
        case "addRowAbove":
          if (row) {
            ...
            for (let i = 0; i < colCount; i++) {
              ...
              cell.style.cssText = "border: 1px solid var(--border-color); padding: 12px; min-width: 100px;"
        ...
        case "addColumnLeft":
          if (cell) {
            ...
            Array.from(table.rows).forEach((row) => {
              ...
              newCell.style.cssText = "border: 1px solid var(--border-color); padding: 12px; min-width: 100px;"
      ...
      cells.forEach((cell) => {
        ...
        ;(cell as HTMLElement).style.backgroundColor = "var(--accent-primary)"
        setTimeout(() => {
  ...
  return (
    <div className={`scrivly-editor ${className} ${isDarkMode ? "dark" : ""} ${isFullscreen ? "fullscreen" : ""}`}>
      <Toolbar
        isActive={isActive}
        currentBlock={currentBlock}
        onAction={(action, value) => {
          if (typeof toolbarActions[action as keyof typeof toolbarActions] === "function") {
            ;(toolbarActions[action as keyof typeof toolbarActions] as Function)(value)
          }
        }}
        ...
        <div className="scrivly-editor-footer">
          ...
          {selectedTable && (
            <div className="table-info">
              <span>Table selected - Right-click for options</span>
            </div>
          )}
        </div>
      )}
        ...
        >
          <div className="context-menu-content">
            <button
              ...
              onClick={() => {
                handleTableOperation("addRowAbove", tableContextMenu.element!)
                setTableContextMenu({ show: false, x: 0, y: 0, element: null })
              }}
            >
              ➕ Add Row Above
            ...
            <button
              className="context-menu-item"
              onClick={() => {
                handleTableOperation("addRowBelow", tableContextMenu.element!)
                setTableContextMenu({ show: false, x: 0, y: 0, element: null })
              }}
            >
              ➕ Add Row Below
            ...
            <button
              className="context-menu-item"
              onClick={() => {
                handleTableOperation("addColumnLeft", tableContextMenu.element!)
                setTableContextMenu({ show: false, x: 0, y: 0, element: null })
              }}
            >
              ➕ Add Column Left
            ...
            <button
              className="context-menu-item"
              onClick={() => {
                handleTableOperation("addColumnRight", tableContextMenu.element!)
                setTableContextMenu({ show: false, x: 0, y: 0, element: null })
              }}
            >
              ➕ Add Column Right
            ...
            <button
              ...
              onClick={() => {
                handleTableOperation("deleteRow", tableContextMenu.element!)
                setTableContextMenu({ show: false, x: 0, y: 0, element: null })
              }}
            >
              🗑️ Delete Row
            ...
            <button
              className="context-menu-item danger"
              onClick={() => {
                handleTableOperation("deleteColumn", tableContextMenu.element!)
                setTableContextMenu({ show: false, x: 0, y: 0, element: null })
              }}
            >
              🗑️ Delete Column
